...
/* TEACHER CHAT */
.tc-msg{display:flex;flex-direction:column;gap:2px;max-width:75%}
.tc-msg.mine{align-self:flex-end;align-items:flex-end}
.tc-msg.theirs{align-self:flex-start;align-items:flex-start}
.tc-bubble{padding:9px 14px;border-radius:18px;font-size:14px;line-height:1.5;word-break:break-word}
.tc-msg.mine .tc-bubble{background:var(--blue);color:#fff;border-bottom-right-radius:4px}
.tc-msg.theirs .tc-bubble{background:var(--parchment);color:var(--ink);border-bottom-left-radius:4px;border:1px solid var(--hairline)}
.tc-meta{font-size:11px;color:var(--ink-48);padding:0 4px}
.tc-student-row{display:flex;align-items:center;gap:12px;padding:14px 20px;border-bottom:1px solid var(--hairline);cursor:pointer;transition:background var(--t)}
.tc-student-row:last-child{border-bottom:none}
.tc-student-row:hover{background:var(--parchment)}
.tc-student-avatar{width:36px;height:36px;border-radius:50%;background:var(--ink);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:600;color:#fff;flex-shrink:0}
.tc-student-info{flex:1}
.tc-student-name{font-size:14px;font-weight:600;color:var(--ink)}
.tc-student-preview{font-size:12px;color:var(--ink-48);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px}
.tc-unread{background:var(--blue);color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:999px}
...
<div id="app-shell">
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <div class="nav-group">
        ...
        <div class="nav-item" data-page="teacher-chat" onclick="navigate('teacher-chat')">
          <svg viewBox="0 0 24 24"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>담임톡
          <span id="teacher-chat-badge" style="display:none;margin-left:auto;background:var(--blue);color:#fff;font-size:10px;font-weight:700;padding:1px 6px;border-radius:999px"></span>
        </div>
      </div>
      <div class="nav-group">
  ...
  <main class="main-content">
    ...
    <div class="page" id="page-report">
      ...
      <div class="page-sub" id="report-period-label">오늘의 학습 현황</div>

      <!-- 기간 탭 -->
      <div class="view-toggle" style="margin-bottom:20px">
        <button class="view-btn active" id="rep-btn-daily" onclick="switchReportPeriod('daily')">일간</button>
        <button class="view-btn" id="rep-btn-weekly" onclick="switchReportPeriod('weekly')">주간</button>
        <button class="view-btn" id="rep-btn-monthly" onclick="switchReportPeriod('monthly')">월간</button>
      </div>

      <!-- 주간/월간일 때만 보이는 헤더 (날짜 범위 + 이전/다음) -->
      <div id="rep-nav" style="display:none;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div style="font-size:15px;font-weight:600;color:var(--ink)" id="rep-range-label"></div>
        <div style="display:flex;gap:8px">
          <button class="btn-ghost" style="padding:5px 12px;font-size:13px" onclick="shiftReportPeriod(-1)">‹</button>
          <button class="btn-ghost" style="padding:5px 12px;font-size:13px" onclick="shiftReportPeriod(1)">›</button>
        </div>
      </div>
      ...
      <div class="grid-2 spacer" style="margin-bottom:16px">
        <div class="card" style="text-align:center;padding:24px">
          <div style="font-size:12px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:var(--ink-48);margin-bottom:8px">총 공부 시간</div>
          <div style="font-size:36px;font-weight:600;letter-spacing:-.03em;color:var(--ink);font-variant-numeric:tabular-nums" id="rep-total">0:00:00</div>
        </div>
        <div class="card" style="text-align:center;padding:24px">
          <div style="font-size:12px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:var(--ink-48);margin-bottom:8px">하루 평균</div>
          <div style="font-size:36px;font-weight:600;letter-spacing:-.03em;color:var(--ink);font-variant-numeric:tabular-nums" id="rep-avg">0:00:00</div>
        </div>
      </div>

      <!-- 과목별 공부 시간 (가로 바) -->
      <div class="card spacer">
        <div style="font-size:15px;font-weight:600;color:var(--ink);margin-bottom:16px">과목별 공부 시간</div>
        <div id="rep-subject-bars" style="display:flex;flex-direction:column;gap:14px"></div>
      </div>

      <!-- 과목 비율 바 -->
      <div class="card spacer">
        <div style="font-size:15px;font-weight:600;color:var(--ink);margin-bottom:14px">과목별 비율</div>
        <div id="rep-ratio-bar" style="height:28px;border-radius:8px;overflow:hidden;display:flex;margin-bottom:12px"></div>
        <div id="rep-ratio-legend" style="display:flex;flex-wrap:wrap;gap:10px 18px"></div>
      </div>

      <!-- 일별 공부 시간 막대 -->
      <div class="card">
        <div style="font-size:15px;font-weight:600;color:var(--ink);margin-bottom:16px">일별 공부 시간</div>
        <div id="rep-daily-chart" style="display:flex;align-items:flex-end;gap:6px;height:120px;padding-bottom:4px"></div>
        <div id="rep-daily-labels" style="display:flex;gap:6px;margin-top:6px"></div>
      </div>
    </div>

    <!-- TEACHER CHAT -->
    <div class="page" id="page-teacher-chat">
      <div class="page-title">담임톡</div>
      <div class="page-sub" id="teacher-chat-sub">담임 선생님과 1:1로 대화하세요</div>

      <!-- 선생님 뷰: 학생 목록 -->
      <div id="teacher-student-list" style="display:none">
        <div class="card" style="padding:0">
          <div id="teacher-student-items"></div>
        </div>
      </div>

      <!-- 채팅창 (학생 뷰 + 선생님이 학생 선택 후) -->
      <div id="teacher-chat-box" style="display:none;flex-direction:column;height:calc(100vh - 200px);min-height:400px">
        <!-- 선생님 뷰에서 뒤로가기 -->
        <div id="teacher-chat-back" style="display:none;margin-bottom:12px">
          <button class="btn-ghost" style="font-size:13px;padding:6px 14px" onclick="showTeacherStudentList()">← 학생 목록</button>
          <span id="teacher-chat-with" style="font-size:15px;font-weight:600;color:var(--ink);margin-left:12px"></span>
        </div>
        <!-- 메시지 목록 -->
        <div class="card" style="flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:10px;margin-bottom:12px" id="teacher-msg-list">
          <div style="text-align:center;padding:24px;color:var(--ink-48);font-size:13px">메시지를 불러오는 중...</div>
        </div>
        <!-- 입력창 -->
        <div style="display:flex;gap:8px">
          <input class="modal-input" id="teacher-msg-input" placeholder="메시지를 입력하세요..." style="flex:1" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendTeacherMessage()}">
          <button class="btn-primary" style="padding:10px 20px;flex-shrink:0" onclick="sendTeacherMessage()">전송</button>
        </div>
      </div>

      <!-- 초기 로딩 전 -->
      <div id="teacher-chat-loading" style="text-align:center;padding:48px;color:var(--ink-48);font-size:13px">로딩 중...</div>
    </div>
    ...
    <div class="page" id="page-admin">
      ...
      <div class="card" id="admin-create-card" style="display:none;margin-bottom:16px">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <input class="modal-input" id="create-student-id" placeholder="학번 (5자리) 또는 teacher" maxlength="10" style="width:160px;font-size:13px;padding:8px 12px">
...
function todayKST(){const d=new Date();d.setTime(d.getTime()+9*60*60*1000);return d.toISOString().slice(0,10);}
...
async function saveName(){
  ...
  const meta={
    ...
    display_name:name,student_id:currentStudentId,
...
// user_metadata를 항상 서버에서 최신값 가져와서 병합 후 저장 (덮어쓰기 방지 공통 헬퍼)
async function updateUserMeta(patch){
  const {data,error:fetchErr}=await sb.auth.getUser();
  if(fetchErr||!data?.user)return{error:fetchErr||new Error('유저 정보 없음')};
  const merged={...data.user.user_metadata,...patch};
  const {error}=await sb.auth.updateUser({data:merged});
  if(!error)currentUser.user_metadata=merged;
  return{error};
}

async function saveResolution(){
  const val=document.getElementById('mp-resolution').value.trim();
  const {error}=await updateUserMeta({resolution:val});
...
async function saveSubjects(){
  ...
  const {error}=await updateUserMeta({suneung_kor:kor,suneung_math:math,suneung_exp1:exp1,suneung_exp2:exp2});
  if(error){toast('저장 실패: '+error.message);return}
  // 정보 업데이트
...
function navigate(page){
  ...
  if(page==='teacher-chat')loadTeacherChat();
}
...
// ── D-DAY ──
function updateDday(){
  const target=new Date('2026-11-19T00:00:00+09:00');
  const nowStr=todayKST();
  const now=new Date(nowStr+'T00:00:00+09:00');
...
async function loadMeal(){
  ...
  document.getElementById('meal-date-label').textContent=`${+m}/${+d}`;
...
function buildSubjectsFromMeta(meta) {
  ...
  const MERGE_MAP = {
    ...
    '화법과작문':'국어','언어와매체':'국어',
    // 수학 선택
    '확률과통계':'수학','미적분':'수학','기하':'수학',
    // 탐구는 그대로 유지 (과목명 자체가 의미있음)
  };
  ...
  const extras = [
    ...
    meta?.suneung_exp1,
    meta?.suneung_exp2
  ].filter(Boolean).map(s => MERGE_MAP[s] || s); // 선택과목 → 기본 과목으로 치환
  // deduplicate
...
let reportOffset=0; // 0=이번 기간, -1=지난 기간, +1=다음 기간

function switchReportPeriod(p){
  reportPeriod=p;reportOffset=0;
  ['daily','weekly','monthly'].forEach(k=>{
    document.getElementById(`rep-btn-${k}`).classList.toggle('active',k===p);
  });
  document.getElementById('rep-nav').style.display=p==='daily'?'none':'flex';
  loadReport();
}

function shiftReportPeriod(dir){
  reportOffset+=dir;loadReport();
}
...
function getReportRange(){
  ...
  if(reportPeriod==='daily'){
    const dt=new Date(y,m-1,d+reportOffset);
    const s=dt.toISOString().slice(0,10);
    return{start:s,end:s,days:1};
  }
  if(reportPeriod==='weekly'){
    const dt=new Date(y,m-1,d);
    const dow=dt.getDay();
    const mon=new Date(dt);mon.setDate(dt.getDate()-(dow===0?6:dow-1)+(reportOffset*7));
    const sun=new Date(mon);sun.setDate(mon.getDate()+6);
    return{start:mon.toISOString().slice(0,10),end:sun.toISOString().slice(0,10),days:7};
  }
  // monthly
  const nm=new Date(y,m-1+reportOffset,1);
  const last=new Date(nm.getFullYear(),nm.getMonth()+1,0);
  return{start:nm.toISOString().slice(0,10),end:last.toISOString().slice(0,10),days:last.getDate()};
}

function fmtSecHMS(s){
  const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sc=s%60;
  return`${h}:${String(m).padStart(2,'0')}:${String(sc).padStart(2,'0')}`;
}

async function loadReport(){
  const {start,end,days}=getReportRange();
  ...
  if(reportPeriod==='daily'){
    ...
    document.getElementById('report-period-label').textContent=`${sy}년 ${sm}월 ${sd}일 (${DAYS_KR[dt.getDay()]}) 학습 현황`;
    document.getElementById('rep-nav').style.display='none';
  } else if(reportPeriod==='weekly'){
    document.getElementById('rep-range-label').textContent=`${sm}월 ${sd}일 ~ ${em}월 ${ed}일`;
    document.getElementById('report-period-label').textContent='주간 학습 현황';
    document.getElementById('rep-nav').style.display='flex';
  } else {
    document.getElementById('rep-range-label').textContent=`${sy}년 ${sm}월`;
    document.getElementById('report-period-label').textContent='월간 학습 현황';
    document.getElementById('rep-nav').style.display='flex';
  }

  // DB에서 해당 기간 세션 로드
  const {data,error}=await sb.from('study_sessions')
    .select('subject,duration_seconds,date,started_at')
  ...
  // 공부한 날 수 (평균 계산용)
  const studyDays=new Set(sessions.map(r=>r.date)).size||1;
  const avgSec=Math.floor(totalSec/studyDays);

  document.getElementById('rep-total').textContent=fmtSecHMS(totalSec);
  document.getElementById('rep-avg').textContent=fmtSecHMS(avgSec);

  // 과목별 집계
  const subMap={};
  sessions.forEach(r=>{
    subMap[r.subject]=(subMap[r.subject]||0)+(r.duration_seconds||0);
  });
  const subColors={};subjects.forEach(s=>subColors[s.name]=s.color);
  const subEntries=Object.entries(subMap).sort((a,b)=>b[1]-a[1]);
  const maxSub=subEntries[0]?.[1]||1;

  // 과목별 가로 바
  document.getElementById('rep-subject-bars').innerHTML=subEntries.length
    ?subEntries.map(([name,sec])=>{
      const pct=Math.round((sec/maxSub)*100);
      const color=subColors[name]||'#7a7a7a';
      return`<div>
        <div style="display:flex;justify-content:space-between;margin-bottom:5px">
          <span style="font-size:13px;color:var(--ink)">${name}</span>
          <span style="font-size:13px;font-weight:600;color:var(--ink);font-variant-numeric:tabular-nums">${fmtSecHMS(sec)}</span>
        </div>
        <div style="height:10px;background:var(--hairline);border-radius:5px;overflow:hidden">
          <div style="height:100%;width:${pct}%;background:${color};border-radius:5px;transition:width .4s ease"></div>
        </div>
      </div>`;
    }).join('')
    :'<div style="text-align:center;padding:16px;color:var(--ink-48);font-size:13px">이 기간에 기록된 공부가 없어요</div>';

  // 과목 비율 바
  const ratioBar=document.getElementById('rep-ratio-bar');
  const ratioLegend=document.getElementById('rep-ratio-legend');
  if(totalSec>0){
    ratioBar.innerHTML=subEntries.map(([name,sec])=>{
      const pct=(sec/totalSec*100).toFixed(1);
      const color=subColors[name]||'#7a7a7a';
      return`<div style="width:${pct}%;background:${color};height:100%"></div>`;
    }).join('');
    ratioLegend.innerHTML=subEntries.map(([name,sec])=>{
      const pct=Math.round(sec/totalSec*100);
      const color=subColors[name]||'#7a7a7a';
      return`<div style="display:flex;align-items:center;gap:5px;font-size:12px;color:var(--ink-80)">
        <div style="width:9px;height:9px;border-radius:50%;background:${color};flex-shrink:0"></div>
        ${name} ${pct}%
      </div>`;
    }).join('');
  } else {
    ratioBar.innerHTML='<div style="width:100%;background:var(--hairline)"></div>';
  ...
  // 기간 내 날짜 배열 생성
  const dateList=[];
  const cur=new Date(start);const endDt=new Date(end);
  while(cur<=endDt){dateList.push(cur.toISOString().slice(0,10));cur.setDate(cur.getDate()+1);}
  const byDate={};
  sessions.forEach(r=>byDate[r.date]=(byDate[r.date]||0)+(r.duration_seconds||0));
  const maxDay=Math.max(...dateList.map(d=>byDate[d]||0),1);
  const chartEl=document.getElementById('rep-daily-chart');
  const labelEl=document.getElementById('rep-daily-labels');
  const today=todayKST();
  chartEl.innerHTML=dateList.map(dt=>{
    const sec=byDate[dt]||0;
    const pct=Math.round((sec/maxDay)*100);
    const isToday=dt===today;
    const [,dm,dd]=dt.split('-').map(Number);
    const dow=DAYS_KR[new Date(dt).getDay()];
    const label=dateList.length<=7?dow:`${dm}/${dd}`;
    return`<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
      ${sec>0?`<div style="font-size:10px;color:var(--ink-48);font-variant-numeric:tabular-nums;white-space:nowrap">${fmtSecHMS(sec)}</div>`:'<div style="font-size:10px;color:var(--ink-48)">0:00</div>'}
      <div style="width:100%;display:flex;align-items:flex-end;height:80px">
        <div style="width:100%;height:${Math.max(pct,sec>0?2:0)}%;background:${isToday?'var(--blue)':'#a5b4fc'};border-radius:4px 4px 0 0;transition:height .4s ease"></div>
      </div>
      <div style="font-size:11px;color:${isToday?'var(--blue)':'var(--ink-48)'};font-weight:${isToday?600:400}">${label}</div>
    </div>`;
  }).join('');
  labelEl.innerHTML='';
}
...
// ── TEACHER CHAT ──
const TEACHER_ID = 'teacher'; // teacher@bugwang3-1.app
let tcSubscription = null;
let tcTargetStudentId = null; // 선생님이 보고 있는 학생 ID

async function loadTeacherChat() {
  document.getElementById('teacher-chat-loading').style.display = 'none';
  if (currentRole === 'teacher') {
    // 선생님 뷰: 학생 목록
    document.getElementById('teacher-chat-sub').textContent = '학생들의 메시지를 확인하세요';
    showTeacherStudentList();
  } else {
    // 학생 뷰: 바로 채팅창
    document.getElementById('teacher-student-list').style.display = 'none';
    document.getElementById('teacher-chat-back').style.display = 'none';
    openTeacherChat(currentStudentId);
  }
}

async function showTeacherStudentList() {
  tcTargetStudentId = null;
  document.getElementById('teacher-student-list').style.display = 'block';
  document.getElementById('teacher-chat-box').style.display = 'none';
  if (tcSubscription) { tcSubscription.unsubscribe(); tcSubscription = null; }

  // 메시지가 있는 학생 목록 조회
  const { data } = await sb.from('teacher_messages')
    .select('student_id, author_name, content, created_at, is_read, sender_role')
    .order('created_at', { ascending: false });

  if (!data || !data.length) {
    document.getElementById('teacher-student-items').innerHTML =
      '<div style="text-align:center;padding:32px;color:var(--ink-48);font-size:13px">아직 메시지가 없어요</div>';
    return;
  }

  // 학생별 최근 메시지 + 안 읽은 수 집계
  const byStudent = {};
  data.forEach(m => {
    if (!byStudent[m.student_id]) byStudent[m.student_id] = { latest: m, unread: 0, name: m.author_name };
    if (!m.is_read && m.sender_role !== 'teacher') byStudent[m.student_id].unread++;
  });

  document.getElementById('teacher-student-items').innerHTML = Object.entries(byStudent)
    .sort((a, b) => new Date(b[1].latest.created_at) - new Date(a[1].latest.created_at))
    .map(([sid, info]) => {
      const name = info.name || sid + '번';
      const preview = info.latest.content.slice(0, 30) + (info.latest.content.length > 30 ? '...' : '');
      const unreadBadge = info.unread > 0 ? `<span class="tc-unread">${info.unread}</span>` : '';
      return `<div class="tc-student-row" onclick="openTeacherChatAsTeacher('${sid}','${name}')">
        <div class="tc-student-avatar">${name[0]}</div>
        <div class="tc-student-info">
          <div class="tc-student-name">${name}</div>
          <div class="tc-student-preview">${preview}</div>
        </div>
        ${unreadBadge}
      </div>`;
    }).join('');
...
async function openTeacherChatAsTeacher(studentId, studentName) {
  tcTargetStudentId = studentId;
  document.getElementById('teacher-student-list').style.display = 'none';
  document.getElementById('teacher-chat-back').style.display = 'flex';
  document.getElementById('teacher-chat-with').textContent = studentName;
  openTeacherChat(studentId);
  // 읽음 처리
  await sb.from('teacher_messages').update({ is_read: true })
    .eq('student_id', studentId).eq('sender_role', 'student');
}

async function openTeacherChat(studentId) {
  document.getElementById('teacher-chat-box').style.display = 'flex';
  document.getElementById('teacher-msg-input').focus();
  await renderTeacherMessages(studentId);

  // 실시간 구독
  if (tcSubscription) tcSubscription.unsubscribe();
  tcSubscription = sb.channel(`teacher_chat_${studentId}`)
    .on('postgres_changes', {
      event: 'INSERT', schema: 'public', table: 'teacher_messages',
      filter: `student_id=eq.${studentId}`
    }, () => renderTeacherMessages(studentId))
    .subscribe();
}

async function renderTeacherMessages(studentId) {
  const { data } = await sb.from('teacher_messages')
    .select('*').eq('student_id', studentId)
    .order('created_at', { ascending: true });
  const el = document.getElementById('teacher-msg-list');
  if (!data || !data.length) {
    el.innerHTML = '<div style="text-align:center;padding:32px;color:var(--ink-48);font-size:13px">아직 대화가 없어요. 먼저 메시지를 보내보세요!</div>';
    return;
  }
  el.innerHTML = data.map(m => {
    const isMine = currentRole === 'teacher' ? m.sender_role === 'teacher' : m.sender_role !== 'teacher';
    const time = new Date(m.created_at).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' });
    return `<div class="tc-msg ${isMine ? 'mine' : 'theirs'}">
      <div class="tc-bubble">${m.content.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>')}</div>
...
async function sendTeacherMessage() {
  ...
  const studentId = isTeacher ? tcTargetStudentId : currentStudentId;
  if (!studentId) return;

  const { error } = await sb.from('teacher_messages').insert({
  ...
  if (error) toast('전송 실패: ' + error.message);
  // 실시간 구독이 자동으로 렌더링함
}

async function checkTeacherUnread() {
  if (currentRole === 'teacher') return;
  const { count } = await sb.from('teacher_messages')
...
async function loadMyRole(){
  ...
  if(badge){
    ...
    else if(currentRole==='teacher'){badge.textContent='선생님';badge.className='admin-role-badge admin';}
    else{badge.style.display='none';}
  }
  // 선생님 답장 알림 체크
  checkTeacherUnread();
...
async function createAccount(){
  ...
  if(!/^\d{5}$/.test(sid)&&sid!=='teacher'){toast('학번은 5자리 숫자 또는 teacher여야 합니다');return}
...
async function saveCalcData(){
  ...
  try{
    const {error}=await updateUserMeta({[CALC_DB_KEY]:JSON.stringify(calcData)});
    if(error)console.error('내신 성적 저장 실패:', error.message);
  }catch(e){console.error('saveCalcData 오류:', e.message);}
}
...
async function renderNews(){
  ...
  if(summaryData&&summaryData.length){
    // 마크다운 → HTML 간단 변환 (볼드, 줄바꿈)
    const md = summaryData[0].summary || '';
    summaryEl.innerHTML = md
